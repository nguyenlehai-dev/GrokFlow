...
from app.models import (
    ApiKey, Domain, FlowJob, Job, Payment, Profile, User,
)

# Gateway models intentionally absent from plxeditor-studio. Stub
# placeholders so the gw_items branch below short-circuits to an
# empty list without a NameError.
...
async def _build(
    ...
    # Gateway LLM panel — plxeditor-studio does NOT ship the gateway
    # product, so this is permanently empty. The FE still renders the
    # group header but the empty-state takes over.
    gw_items: list[AppItem] = []
