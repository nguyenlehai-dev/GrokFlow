...
export function FlowRequestsPage() {
  ...
  const retry = useMutation({
    ...
    onSuccess: () => {
      toast("Đã re-fire job", "info");
      qc.invalidateQueries({ queryKey: ["flow-jobs"] });
    },
    onError: () => toast("Retry thất bại", "error"),
  });
      ...
      <div className="card flex flex-wrap items-end gap-3 p-3">
        ...
        <label className="block min-w-[150px]">
          ...
          >
            ...
            {TOOLS.map((t) => (
              <option key={t.slug} value={t.slug}>{t.shortLabel}</option>
            ))}
          </select>
        </label>
        <label className="block min-w-[150px]">
    ...
    <div className="card flex flex-col gap-3 p-4">
        ...
        <div className="flex items-center gap-2">
          {(job.status === "failed" || job.status === "completed") && job.input_files && job.input_files.length > 0 && (
            <button
              ...
              onClick={onRetry}
              className="btn-ghost text-xs inline-flex items-center gap-1"
              title="Chạy lại với cùng input + params"
            >
              <PlayCircle size={14} /> Retry
            </button>
          )}
          {job.status === "completed" && job.output_url && (
            ...
            >
              <Download size={14} />
              {job.file_size ? `${(job.file_size / 1024 / 1024).toFixed(1)} MB` : "Tải"}
            </a>
          )}
        </div>
      </div>
      ...
      {job.status === "failed" && job.error_message && (
        <div className="rounded-md bg-rose-50 px-3 py-2 text-xs text-rose-700">
          <span className="font-semibold uppercase tracking-wide">Lỗi:</span>{" "}
