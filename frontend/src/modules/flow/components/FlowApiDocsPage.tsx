  ...
  return (
    <div className="group relative">
      ...
      <button
        type="button"
        onClick={onCopy}
        className="absolute right-2 top-2 inline-flex items-center gap-1 rounded-md border border-slate-700 bg-slate-900 px-2 py-1 text-[10px] text-slate-300 opacity-0 transition hover:border-violet-500/60 group-hover:opacity-100"
      >
...
function ToolCard({ spec }: { spec: ToolSpec }) {
  return (
    <article className="rounded-2xl border border-slate-800 bg-slate-900/40">
      <header className="border-b border-slate-800 px-5 py-4">
        <div className="flex items-center gap-2">
          <MethodBadge method={spec.method} />
          <code className="text-sm font-semibold text-slate-100">{spec.endpoint}</code>
        </div>
                ...
                {spec.params.map((p) => (
                  <tr key={p.name}>
                    <td className="px-3 py-2 align-top">
                      <code className="text-xs font-semibold text-slate-100">{p.name}</code>
                      {p.required && (
                        <span className="ml-1.5 rounded bg-rose-500/15 px-1 py-0.5 text-[10px] font-bold uppercase text-rose-300">
                          required
                        </span>
                      )}
                    </td>
                    ...
                    <td className="px-3 py-2 align-top text-xs text-slate-300">{p.desc}</td>
                  </tr>
                ))}
              </tbody>
            </table>
