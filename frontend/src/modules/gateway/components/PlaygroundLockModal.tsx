  ...
  const verify = useMutation({
    ...
    onSuccess: ({ data }, key) => {
      if (data.verified) {
        ...
        toast(t("gateway.lock_modal_verified_toast"), "success");
        setOpen(false);
      } else {
        toast(t("gateway.lock_modal_invalid_toast"), "error");
      }
    },
      ...
      <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <h2 className="text-xl font-bold text-slate-800 mt-1">{t("gateway.lock_modal_title")}</h2>
            <p className="text-sm text-slate-600 mt-2">
              {t("gateway.lock_modal_description")}
            </p>
          </div>
        </div>
        ...
        {open ? (
          <div className="mt-5 space-y-3">
            ...
            <div className="flex gap-2 justify-end">
              ...
              >
                {verify.isPending ? (
                  <><Loader2 size={14} className="animate-spin" /> {t("gateway.lock_modal_verifying")}</>
                ) : (
                  <><CheckCircle2 size={14} /> {t("gateway.lock_modal_verify_key")}</>
                )}
              </button>
            </div>
            <p className="text-xs text-slate-500">
              {t("gateway.lock_modal_no_key_prefix")} <a href="/gateway/gateway-keys" className="text-brand-600 underline">/gateway/gateway-keys</a> {t("gateway.lock_modal_no_key_suffix")}
            </p>
          </div>
        ) : (
          <button
